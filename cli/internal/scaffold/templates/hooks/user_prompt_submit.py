...
def _rotate_if_corrupt(db_path: Path) -> None:
    """Move *db_path* to a timestamped backup if it fails a SELECT 1 probe.

    No-op when the file does not exist (fresh workspace) or when the DB is
    healthy.  If the DB is corrupt or locked beyond recovery, the backup
    preserves the old file and the caller can create a fresh DB at the same
    path.
    """
    ...
    try:
        ...
        backup = db_path.with_name(f"tracer.{stamp}.bak.db")
        try:
